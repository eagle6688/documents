...
select p.id, u.name userName, p.type, p.cost, p.medium, p.time, p.description, p.created, p.updated
from payment p
inner join user u on p.userid = u.id and u.deleted = 0
where p.deleted = 0;

#Select all shopping for display.
select p.id, u.name userName, p.type, s.channel, p.cost, p.medium, p.time, p.description, p.created, p.updated
from payment p
...
select p.id, fu.name userName, p.type, t.to_user_id toUserId, tu.name toUser, p.cost, p.medium, p.time, p.description, p.created, p.updated
